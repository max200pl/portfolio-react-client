...
test.describe("ModalHireMe", () => {
    test.beforeEach(async ({ page }) => {
        await page.goto("/");
        await page
            .getByRole("button", { name: /hire me/i })
            .first()
            .click();
    ...
    test("shows email validation error for incorrect email", async ({
        page,
    }) => {
        await page.getByLabel("Email").fill("not-an-email");
        await page.getByRole("button", { name: "Send" }).click();
        await expect(
            page.getByText("Please write correct email")
        ).toBeVisible();
    });
    ...
    test("fills and submits form successfully", async ({ page }) => {
        ...
        await page
            ...
            .fill("I want to hire you for a project");
